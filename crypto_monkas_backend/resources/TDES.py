...
_pythonMajorVersion = version_info[0]
...
class des(_baseDes):
	...
	def crypt(self, data, crypt_type):
		...
		while i < len(data):
			...
			block = self.__String_to_BitList(bytes(data[i:i+8]))
			processed_block = self.__des_crypt(block, crypt_type)

			# Add the resulting crypted block to our list
			#d = self.__BitList_to_String(processed_block)
			#result.append(d)
			result += list(map(int, self.__BitList_to_String(processed_block)))
			#dict[data[i:i+8]] = d
			i += 8
...
def handleModes(cypher, input, IV = [0] * 8, mode = 'ECB', encdec = "enc"):
    ...
    if mode == 'ECB':
        ...
        while len(result) < len(input):
            ...
            processed = operation[encdec](block)
            if encdec == "dec":
                processed = list(map(lambda x, y: x ^ y, processed, iv))
                iv = block
            else:
                iv = processed
            result += processed
        ...
        forXOR = []
        while i < len(input):
            iv = cypher.encrypt(iv)
            forXOR += iv
            i += 8
        return list(map(lambda x, y: x ^ y, input, forXOR))
    elif mode == 'CFB':
        iv = IV
        while i < len(input):
            block = input[i:i+8]
            processed = cypher.encrypt(iv)
            processed = list(map(lambda x, y: x ^ y, block, processed))
            iv = processed if (encdec == "enc") else block
            result += processed
            i += 8
        return result
    else:
    ...
    def encryption(filename: str, key: bytes, iv : bytes, mode : str) -> str:
        ...
        for channel in range(3):
            for j in range(image.shape[1]):
                #print(image[:, j, channel])
                image[:, j, channel] = handleModes(triple_des(key), bytes(image[:, j, channel]), iv, mode, encdec = 'enc')
                #if (j % 100 == 0):
                #    print(progres[(j % 400) // 100], j, image.shape[1], channel)
...
class TDESDec(Resource):
    ...
    def decryption(filename: str, key: bytes, iv:bytes, mode:str) -> str:
        ...
        for channel in range(3):
            for j in range(image.shape[1]):
                image[:, j, channel] = handleModes(triple_des(key), bytes(image[:, j, channel]), iv, mode, encdec = 'dec')
                #if (j % 100 == 0):
                #     print(progres[(j % 400) // 100], j, image.shape[1], channel)
        image = Image.fromarray(image)
